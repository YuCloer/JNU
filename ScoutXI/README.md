...
- 阵型编辑器扩展为 13 套常用体系（含菱形中场、双前腰、三中卫与五后卫）；所有阵型均固定 11 个现实战术区域，并收紧了不合理的位置兼容。
- 进入首发或替补的球员会立即从球员池隐藏；球员池支持姓名/俱乐部搜索和位置筛选，移除使用右键且不再弹出确认框。
- 全局球员名称采用英文显示；头像优先使用当前俱乐部可核验的远程真人照片。公开源没有可靠照片时显示无文字的黑色默认人像，不保存图片到本地。
- 移除了旧的本地“综合评分”，避免将演示数值误作实时或官方评分；焦点卡的 `5G` 等标识仅表示本赛季进球数。
- 五大联赛名单继续由 football-data.org 同步；新增葡超、苏超、荷甲、土超、美职联与沙特联的代表性俱乐部补充数据，并保留梅西与 C 罗等跨联赛高声望球员。补充联赛使用公开 TheSportsDB 目录，免费接口每队仅提供有限球员资料，因此不宣称其为完整联赛名单。

## 范围

已实现：搜索与筛选、球员详情、俱乐部赛季阵容、13 套位置约束阵型画布、首发/替补/队长、保存、收藏、个人球探报告、同步状态。
